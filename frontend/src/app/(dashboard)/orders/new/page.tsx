...
import LocationPicker, { StoreLocationStatus, useStoreLocation } from '@/components/maps/LocationPicker';
...
function NewOrderForm() {
  ...
  const { store, loading: storeLoading, error: storeError, retry: retryStore } = useStoreLocation();
  ...
  useEffect(() => {
    api.get<{ data: Service[] }>('/api/services').then((res) => setServices(res.data));
  }, []);
  ...
  function canGoStep2() {
    return Boolean(serviceId && weightQty > 0);
  }
